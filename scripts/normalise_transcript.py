...
def EN_spacy(spacy_doc):
	""" On cherche les EN pour mieux borner les débuts de phrases.
		if isupper() and not in list_EN alors début de phrase
		pour éviter quand on est dans une phrase, que par ex.
		le segment "Madame Maillet." soit considéré comme une phrase complète
	"""
	list_EN = []
	...
	print(len(list_EN)) # 1539
	set_EN = set(list_EN)
	print(len(set_EN)) # 480
	# print(set_EN)
	motif = re.compile(".?ez-moi")
	list_sup = [e for e in set_EN if motif.findall(e)]
	print(list_sup)
	for e in list_sup:
		set_EN.discard(e)
	print(len(set_EN))
